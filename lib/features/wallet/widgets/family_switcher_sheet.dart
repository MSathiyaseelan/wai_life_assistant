  ...
  Future<void> _save() async {
    ...
    if (!AuthService.instance.isLoggedIn) {
      ...
      if (_isEdit) {
        ...
        widget.existing!.members
          ..clear()
          ..addAll(_members);
        final idx = familyWallets.indexWhere(
          (w) => w.id == widget.existing!.id,
        );
        if (idx >= 0) {
    ...
    try {
      if (_isEdit) {
        ...
        // Members removed by the user
        for (final removed in widget.existing!.members.where(
          (m) => !currentIds.contains(m.id),
        )) {
        ...
        // Save members the user added (creator is already added by the RPC;
