...
class IntentClassifier {
  ...
  QuestionIntent classify(String question) {
    ...
    // Wallet signals
    if (RegExp(
      r'spend|spent|expense|income|salary|balance|money|₹|paid|transaction|budget|earn|cash|transfer|lend|borrow|split|finance|credit|debit',
    ).hasMatch(q)) {
    ...
    // Pantry signals
    if (RegExp(
      r'pantry|grocery|groceries|food|cook|recipe|meal|ingredient|eat|basket|shopping|buy|fridge',
    ).hasMatch(q)) {
    ...
    // PlanIt signals
    if (RegExp(
      r'task|todo|to-do|plan|bill|remind|schedule|appointment|upcoming|due|deadline|wish|note|pending',
    ).hasMatch(q)) {
    ...
    // Functions signals
    if (RegExp(
      r'function|event|wedding|birthday|occasion|ceremony|attend|party|celebration',
    ).hasMatch(q)) {
  ...
  TimeRange _detectTimeRange(String q) {
    if (RegExp(r'today|tonight|this morning').hasMatch(q)) return TimeRange.today;
    if (RegExp(r'this week|week').hasMatch(q)) return TimeRange.thisWeek;
    if (RegExp(r'last month|previous month').hasMatch(q)) return TimeRange.lastMonth;
  ...
  QueryType _detectQueryType(String q) {
    if (RegExp(r'how much|total|balance|amount').hasMatch(q)) return QueryType.specific;
    if (RegExp(r'compare|vs|versus|difference|between').hasMatch(q)) return QueryType.comparison;
    if (RegExp(r'suggest|recommend|should i|what if|tip|advice|help me').hasMatch(q)) return QueryType.suggestion;
    if (RegExp(r'will|predict|forecast|next month|future').hasMatch(q)) return QueryType.prediction;
    if (RegExp(r'summarise|summary|overview|all|report').hasMatch(q)) return QueryType.summary;
