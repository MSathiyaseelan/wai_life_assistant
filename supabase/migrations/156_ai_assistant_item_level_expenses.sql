-- ============================================================
-- WAI AI Assistant — item-level expense lookups
--
-- Bug: "how much spend on coffee last month" returned "no data".
-- HOUSEHOLD CONTEXT only ever carried the top-3 expense categories
-- by total and the 5 most-recent transactions (context_fetcher.dart).
-- Coffee has no category of its own — it's folded into "Food" — so
-- it never appeared in either slice, and the model correctly (per
-- its own "don't invent data" rule) said there was no data, even
-- though matching rows existed.
--
-- Client fix (context_fetcher.dart): HOUSEHOLD CONTEXT now also
-- includes `all_expenses_this_period`, every individual expense line
-- (title, amount, category) for the requested period, not just the
-- top categories/most-recent few. This prompt update tells the model
-- to actually search that list for item-specific questions instead
-- of only reading the pre-aggregated top_expense_categories.
-- ============================================================
...
- top_expense_categories is only the top 3 categories by total — it will NOT
  mention specific items (e.g. coffee, snacks) that are folded into a broader
  category (e.g. Food). For any question about a SPECIFIC item, merchant, or
  keyword (not a whole category), search the individual line items in
  all_expenses_this_period (title/category/amount per transaction) for
  matches and sum them yourself — do not conclude "no data" just because
  the item isn't one of the top_expense_categories or recent_transactions.
  Only say data is missing if no matching line item exists in
  all_expenses_this_period either.
