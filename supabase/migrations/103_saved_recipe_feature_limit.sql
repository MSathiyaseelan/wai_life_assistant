-- Migration 103: Add 'saved_recipe' to the shared feature-limit resolver.
--
-- plan_limits.pantry_recipes_max has existed since 054_subscription_system.sql
-- (default 10 on personal_free) but was never enforced anywhere — Recipe Box
-- had no limit at all on any plan. This is a standing count cap (how many
-- active recipes currently exist for the wallet), not a monthly usage
-- counter, so — like wallet_custom_categories_max — it's read via
-- get_effective_feature_limit directly rather than check_feature_limit;
-- the client compares it against the wallet's current (non-deleted) recipe
-- count, so deleting a recipe frees up a slot for another.
